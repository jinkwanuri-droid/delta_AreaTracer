    ...
    <div ref={ref} className="print-container-root w-full bg-white text-slate-800 printable-mode" style={{ fontFamily: '"Pretendard", "Inter", sans-serif' }}>
      <style>{`
        ...
        @media print {
          @page {
            size: A4 landscape;
            margin: 0 !important; /* 브라우저 자체 여백 완전 제거 */
          }
          html, body {
            margin: 0 !important;
            padding: 0 !important;
          ...
          .print-container-root {
            ...
            background: #ffffff !important;
            overflow: visible !important;
            font-size: 0 !important; /* 슬라이드 사이의 미세한 공백으로 인한 페이지 넘침 원천 차단 */
          ...
          .pdf-slide-container {
            width: 296mm !important; /* 미세한 오차 방지를 위해 1mm 축소 */
            height: 208mm !important; /* A4 가로(210mm) 보다 작게 설정하여 페이지 넘침 방지 */
                  ...
                  <div key={`${floorName}-p${currentPageNum}`} className="pdf-slide-container bg-white">
                    ...
                    <div className="flex justify-between items-baseline mb-[1.5mm] px-1">
                      ...
                      <div className="border border-slate-300 style-table-pdf-container">
                        <table className="w-full text-slate-800 border-collapse table-fixed text-[6.5pt] font-sans" style={{ width: '100%', minWidth: '100%', letterSpacing: '-0.05em' }}>
                          <colgroup>
                            <col style={{ width: '42px' }} />
                            <col style={{ width: '140px' }} /> {/* 실명 너비를 125px에서 140px로 추가 확장 */}
                            {stages.map((s) => (
                          ...
                          <thead>
                            <tr className="bg-slate-200 border-b border-slate-350 text-slate-800">
                              <th rowSpan={2} className="py-1 px-1 text-center font-extrabold text-[7.5pt] border-r border-slate-300">NO.</th>
                              <th rowSpan={2} className="py-1 px-2 text-left font-extrabold text-[7.5pt] border-r border-slate-300">ROOM NAME</th>
                              {stages.map((s, idx) => {
                                ...
                                return (
                                  <th key={s.id} colSpan={3} className={clsx(
                                    "py-0.5 px-1 text-center font-extrabold border-r border-b border-slate-300 text-[6.5pt]",
                                    headBg
                                  )}>
                                    {s.name}
                                  </th>
                              ...
                              <th rowSpan={2} className="py-1 px-1 text-center font-extrabold text-[7.5pt] border-r border-slate-300 leading-tight w-[10%]">
                                증감<br/>
                                <span className="text-[5.5pt] font-bold text-slate-500 font-mono block mt-0.5 whitespace-nowrap">(실시-중간)</span>
                              </th>
                              <th rowSpan={2} className="py-1 px-2 text-center font-extrabold text-[7.5pt] col-note-print w-[20%]">NOTE</th>
                            </tr>
                              ...
                              {stages.map((s, idx) => {
                                ...
                                return (
                                  <React.Fragment key={`${s.id}-sub`}>
                                    <th className={clsx("py-0.5 px-0.5 text-center font-extrabold border-r border-slate-200 text-[6pt] tracking-tighter", subHeadBg)}>Net</th>
                                    <th className={clsx("py-0.5 px-0.5 text-center font-extrabold border-r border-slate-200 text-[6pt] tracking-tighter", subHeadBg)}>Qty</th>
                                    <th className={clsx("py-0.5 px-1 text-center font-black border-r border-slate-200 text-[6pt] tracking-tighter", isPracticeStage ? "bg-[#F3E8FF] text-purple-950" : "bg-[#CBD5E1] text-slate-900")}>Total</th>
                                  </React.Fragment>
                                );
                              })}
                            </tr>
                        ...
                        <tbody className="divide-y divide-slate-200 bg-white">
                          {chunkRows.map((row, idx) => {
                              ...
                              return (
                                <React.Fragment key={row.id}>
                                  ...
                                  <tr key={row.id} className="bg-slate-100/70 border-b border-slate-300">
                                    <td colSpan={2 + stages.length * 3 + 2} className="py-0.5 px-2.5 font-extrabold text-[#1E293B] text-[7.5pt] border-b border-slate-300">
                            ...
                            if (row.isSummary) {
                              return (
                                <tr key={row.id} className="bg-slate-50/80 font-bold border-y border-slate-200 text-slate-950 text-[6.5pt]">
                                  ...
                                  {stages.map((s) => {
                                    ...
                                    return (
                                      <React.Fragment key={s.id}>
                                        ...
                                        <td className="py-0.5 px-0.5 text-center text-slate-400 text-[6pt] border-b border-slate-300"></td>
                                        <td className="py-0.5 px-0.5 text-center text-slate-400 text-[6pt] border-b border-slate-300"></td>
                            ...
                            // 일반 데이터 행 렌더링
                            return (
                              <tr key={row.id} className="hover:bg-slate-50/20 text-[6.5pt]" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                                <td className="py-0.5 px-1 text-center text-slate-500 font-mono border-r border-slate-200 whitespace-nowrap text-[6pt] tracking-tighter">{row.no}</td>
                                <td className="py-0.5 px-2 text-left text-slate-800 font-semibold border-r border-slate-200 leading-normal whitespace-normal text-[6.5pt]" style={{ wordBreak: 'break-word', wordWrap: 'break-word' }}>{row.name}</td>
                                {stages.map((s) => {
                                  ...
                                  const totalCellBg = isPracticeStage ? "bg-[#F3E8FF]/30 text-purple-950 font-bold text-[6.5pt]" : "bg-slate-100/50 text-[#1E293B] text-[6.5pt]";

                                  return (
                                    <React.Fragment key={s.id}>
                                      <td className={clsx("py-0.5 px-0.5 text-right text-slate-500 font-mono border-r border-slate-200 text-[6pt]", cellBg)}>
                                        {isEmpty ? "" : formatNum(row[`${s.id}_unitArea`], s.id)}
                                      </td>
                                      <td className={clsx("py-0.5 px-0.5 text-center text-slate-500 font-mono border-r border-slate-200 text-[6pt]", cellBg)}>
                                        {isEmpty ? "" : formatQty(row[`${s.id}_quantity`])}
                                      </td>
                                      <td className={clsx("py-0.5 px-1 text-right font-mono font-semibold border-r border-slate-200 text-[6.5pt]", totalCellBg)}>
                                ...
                                <td className={clsx(
                                  "py-0.5 px-1 text-right font-mono font-bold border-r border-slate-200 text-[6.5pt]",
                                  row.variance > 0 ? "text-blue-600" : row.variance < 0 ? "text-red-500" : "text-slate-400"
                                )}>
                                ...
                                <td className="py-0.5 px-2 text-left text-slate-600 font-normal leading-normal whitespace-pre-wrap border-slate-200 text-[6pt]" style={{ wordBreak: 'break-word', wordWrap: 'break-word' }}>
