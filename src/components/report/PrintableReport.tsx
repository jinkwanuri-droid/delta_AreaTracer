    ...
    <div ref={ref} className="print-container-root w-full bg-white text-slate-800 printable-mode" style={{ fontFamily: '"Pretendard", "Inter", sans-serif' }}>
      <style>{`
        ...
        .printable-mode, .printable-mode * {
          font-family: 'Pretendard', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Apple SD Gothic Neo', 'Malgun Gothic', 'Apple Color Emoji', 'Segoe UI Emoji', sans-serif !important;
        }
        ...
        <>
          {(() => {
            ...
            return sortedFloorNames.map((floorName) => {
                  ...
                  <div key={`${floorName}-p${currentPageNum}`} className="pdf-slide-container bg-white">
                    ...
                    <div className="flex justify-between items-baseline mb-[1.5mm] px-1">
                     ...
                     <div className="border border-slate-300 rounded-lg overflow-hidden style-table-pdf-container">
                       <table className="w-full text-slate-800 border-collapse table-fixed text-[1.0px] font-sans" style={{ width: '100%', minWidth: '100%', letterSpacing: '-0.1em' }}>
                         <colgroup>
                         ...
                         <thead>
                           <tr className="bg-slate-200 border-b border-slate-350 text-slate-800">
                             <th rowSpan={2} className="py-1 px-1 text-center font-extrabold text-[2.0px] border-r border-slate-300">NO.</th>
                             <th rowSpan={2} className="py-1 px-2 text-left font-extrabold text-[2.0px] border-r border-slate-300">ROOM NAME</th>
                             {stages.map((s, idx) => {
                               ...
                               return (
                                 <th key={s.id} colSpan={3} className={clsx(
                                   "py-0.5 px-1 text-center font-extrabold border-r border-b border-slate-300 text-[1.5px]",
                                   headBg
                                 )}>
                                   {s.name}
                                 </th>
             ...
             <th rowSpan={2} className="py-1 px-1 text-center font-extrabold text-[2.0px] border-r border-slate-300 leading-tight w-[10%]">
                               증감<br/>
                               <span className="text-[1px] font-bold text-slate-500 font-mono block mt-0.5 whitespace-nowrap">(실시-중간)</span>
                             </th>
                             <th rowSpan={2} className="py-1 px-2 text-center font-extrabold text-[2.0px] col-note-print w-[28%]">NOTE</th>
                           </tr>
                             ...
                             {stages.map((s, idx) => {
                               ...
                               return (
                                 <React.Fragment key={`${s.id}-sub`}>
                                   <th className={clsx("py-0.5 px-0.5 text-center font-extrabold border-r border-slate-200 text-[1.0px] tracking-tighter", subHeadBg)}>Net</th>
                                   <th className={clsx("py-0.5 px-0.5 text-center font-extrabold border-r border-slate-200 text-[1.0px] tracking-tighter", subHeadBg)}>Qty</th>
                                   <th className={clsx("py-0.5 px-1 text-center font-black border-r border-slate-200 text-[1.0px] tracking-tighter", isPracticeStage ? "bg-[#F3E8FF] text-purple-950" : "bg-[#CBD5E1] text-slate-900")}>Total</th>
                                 </React.Fragment>
                               );
                             })}
                           </tr>
                        ...
                        <tbody className="divide-y divide-slate-200 bg-white">
                          {chunkRows.map((row, idx) => {
                              ...
                              return (
                                <React.Fragment key={row.id}>
                                  ...
                                  <tr key={row.id} className="bg-slate-100/70 border-b border-slate-300">
                                    <td colSpan={2 + stages.length * 3 + 2} className="py-0.5 px-2.5 font-extrabold text-[#1E293B] text-[5.0px] border-b border-slate-300">
                                      <div className="flex items-center">
                                        ...
                                        {row.deptName}
                                        {row.wardCount > 1 && (
                                          <span className="ml-2 px-1 py-0.5 text-[3.5px] bg-[#EEF2F6] text-slate-600 border border-slate-200 rounded font-bold">
                            ...
                            if (row.isSummary) {
                              return (
                                <tr key={row.id} className="bg-slate-50/80 font-bold border-y border-slate-350 text-slate-950 text-[1.5px]">
                                  <td className="py-0.5 px-1 border-r border-slate-200 border-b border-slate-400"></td>
                                  <td className="py-0.5 px-2 text-left font-extrabold border-r border-slate-200 border-b border-slate-400">[{row.deptName} 소계]</td>
                                  {stages.map((s) => {
                                    const isPracticeStage = s.name.includes("실기") || s.name.includes("실시") || s.id === "s5";
                                    ...
                                    return (
                                      <React.Fragment key={s.id}>
                                        <td className="py-0.5 px-0.5 text-center border-r border-slate-200 text-slate-400 text-[1.0px] border-b border-slate-400">-</td>
                                        <td className="py-0.5 px-0.5 text-center border-r border-slate-200 text-slate-400 text-[1.0px] border-b border-slate-400">-</td>
                                        <td className={clsx("py-0.5 px-1 text-right border-r border-slate-200 font-extrabold text-[1.5px] border-b border-slate-400", subBg)}>
                                  ...
                                  <td className={clsx(
                                    "py-0.5 px-1 text-right font-extrabold border-r border-slate-200 text-[1.5px] border-b border-slate-400",
                                    row.variance > 0 ? "text-blue-600" : row.variance < 0 ? "text-red-500" : "text-slate-400"
                                  )}>
                                    {row.variance > 0 ? "+" : ""}{formatNum(row.variance)}
                                  </td>
                                  <td className="py-0.5 px-2 border-slate-200 border-b border-slate-400"></td>
                                </tr>
                              );
                            }
